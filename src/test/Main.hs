...
conversion = testGroup "conversion"
  [ testCase "Weather metric to imperial conversion" $
      (Weather (TimeOfDay 1 51 0) (Wind 11.104473600000002) (Temperature 0.5555555555555556) :: Weather Metric)
        @=?
        convert ((Weather (TimeOfDay 1 51 0) (Wind 6.9) (Temperature 33.0)) :: Weather Imperial)
  , testCase "Weather metric to imperial conversion" $
      (Weather (TimeOfDay 1 51 0) (Wind 1.8641135699999998) (Temperature 39.019999999999996) :: Weather Imperial)
        @=?
        convert ((Weather (TimeOfDay 1 51 0) (Wind 3.0) (Temperature 3.9)) :: Weather Metric)
...
parsing = testGroup "parsing"
  [ testCase "A basic parse" $
      (Right (Weather {timeUtc = TimeOfDay 1 51 0, wind = Wind 5.556, temperature = Temperature 3.9}))
  ...
  , testCase "Variable wind using 2 digits: VRB06KT" $
      (Right (Weather {timeUtc = TimeOfDay 0 51 0, wind = Wind 11.112, temperature = Temperature 6.7}))
  ...
  , testCase "Variable wind using 3 digits: VRB106KT" $
      (Right (Weather {timeUtc = TimeOfDay 0 51 0, wind = Wind 196.312, temperature = Temperature 6.7}))
  ...
  , testCase "Wind gust using 2 digits: 28008G17KT" $
      (Right (Weather {timeUtc = TimeOfDay 16 51 0, wind= Wind 14.816, temperature = Temperature 9.4}))
  ...
  , testCase "Temperature data only in 05/04 format" $
      (Right (Weather {timeUtc = TimeOfDay 3 50 0, wind = Wind 14.816, temperature = Temperature 5.0}))
  ...
  , testCase "Temperature data only in M02/M02 format" $
      (Right (Weather {timeUtc = TimeOfDay 3 50 0, wind = Wind 14.816, temperature = Temperature (-2.0)}))
        @=? (parse "EKCH 310350Z AUTO 15008KT 9999 OVC032/// M02/M02 Q1009 NOSIG")

  , testCase "Wind speed in MPS using 2 digits: 15004MPS" $
      (Right (Weather {timeUtc = TimeOfDay 3 50 0, wind = Wind 14.4, temperature = Temperature 5.0}))
  ...
  , testCase "No wind info at all" $
      (Right (Weather {timeUtc = TimeOfDay 20 51 0, wind = Wind 0.0, temperature = Temperature 9.4}))
