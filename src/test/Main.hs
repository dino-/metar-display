...
conversion = testGroup "conversion"
  [ testCase "Weather metric to imperial conversion" $
      (Weather (Station "FOOO") (TimeOfDay 1 51 0) (Wind 11.104473600000002) (Gust (Wind 50.266250496000005)) (Temperature 0.5555555555555556) :: Weather Metric)
        @=?
        convert ((Weather (Station "FOOO") (TimeOfDay 1 51 0) (Wind 6.9) (Gust (Wind 31.234)) (Temperature 33.0)) :: Weather Imperial)
  , testCase "Weather metric to imperial conversion" $
      (Weather (Station "FOOO") (TimeOfDay 1 51 0) (Wind 1.8641135699999998) (Gust (Wind 19.40790774846)) (Temperature 39.019999999999996) :: Weather Imperial)
        @=?
        convert ((Weather (Station "FOOO") (TimeOfDay 1 51 0) (Wind 3.0) (Gust (Wind 31.234)) (Temperature 3.9)) :: Weather Metric)
...
parsing = testGroup "parsing"
  [ testCase "A basic parse" $
      (Right (Weather {station = Station "KRDU", timeUtc = TimeOfDay 1 51 0, wind = Wind 5.556, gust = NoGust, temperature = Temperature 3.9}))
  ...
  , testCase "Variable wind using 2 digits: VRB06KT" $
      (Right (Weather {station = Station "KRDU", timeUtc = TimeOfDay 0 51 0, wind = Wind 11.112, gust = NoGust, temperature = Temperature 6.7}))
  ...
  , testCase "Variable wind using 3 digits: VRB106KT" $
      (Right (Weather {station = Station "KRDU", timeUtc = TimeOfDay 0 51 0, wind = Wind 196.312, gust = NoGust, temperature = Temperature 6.7}))
  ...
  , testCase "Wind with gust in knots using 2 digits: 28008G17KT" $
      (Right (Weather {station = Station "KRDU", timeUtc = TimeOfDay 16 51 0, wind = Wind 14.816, gust = Gust (Wind 31.484), temperature = Temperature 9.4}))
  ...
  , testCase "Wind with gust in knots using 3 digits: 28008G107KT" $
      (Right (Weather {station = Station "KRDU", timeUtc = TimeOfDay 16 51 0, wind = Wind 14.816, gust = Gust (Wind 198.16400000000002), temperature = Temperature 9.4}))
  ...
  , testCase "Temperature data only in 05/04 format" $
      (Right (Weather {station = Station "EKCH", timeUtc = TimeOfDay 3 50 0, wind = Wind 14.816, gust = NoGust, temperature = Temperature 5.0}))
  ...
  , testCase "Temperature data only in M02/M02 format" $
      (Right (Weather {station = Station "EKCH", timeUtc = TimeOfDay 3 50 0, wind = Wind 14.816, gust = NoGust, temperature = Temperature (-2.0)}))
        @=? (parse "EKCH 310350Z AUTO 15008KT 9999 OVC032/// M02/M02 Q1009 NOSIG")

  , testCase "Wind in MPS using 2 digits: 15004MPS" $
      (Right (Weather {station = Station "EKCH", timeUtc = TimeOfDay 3 50 0, wind = Wind 14.4, gust = NoGust, temperature = Temperature 5.0}))
  ...
  , testCase "Wind in MPS with gust using 2 digits: 15004G12MPS" $
      (Right (Weather {station = Station "EKCH", timeUtc = TimeOfDay 3 50 0, wind = Wind 14.4, gust = Gust (Wind 43.2), temperature = Temperature 5.0}))
  ...
  , testCase "Wind in MPS with gust using 3 digits: 150004G102MPS" $
      (Right (Weather {station = Station "EKCH", timeUtc = TimeOfDay 3 50 0, wind = Wind 14.4, gust = Gust (Wind 367.2), temperature = Temperature 5.0}))
        @=? (parse "EKCH 310350Z AUTO 150004G102MPS 9999 OVC032/// 05/04 Q1009 NOSIG")

  , testCase "No wind info at all" $
      (Right (Weather {station = Station "KRDU", timeUtc = TimeOfDay 20 51 0, wind = Wind 0.0, gust = NoGust, temperature = Temperature 9.4}))
